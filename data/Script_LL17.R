...
#Plot specimens based on 'hand webbing degree'
hand_cols <- c('#d73027' , '#fdae61' , '#e0f3f8' , '#74add1' , 'midnightblue' , 'grey70')

pdf('sagittal_doming_left_coronal_doming_right_webbing.pdf', useDingbats = F, width = 18, height = 9)

#dev.new(width = 18, height = 9)
par(mfrow = c(1,2))

#Sagittal doming plot
plot(specimen.info$Length.height ~ specimen.info$Carapace.plastron_width, pch = 21, 
     bg = hand_cols [as.numeric(as.factor(specimen.info$Hand_webbing))] , 
     cex = 2.1, xlab = "Carapace width-plastron width ratio" , 
     ylab = "Carapace length-height ratio (sagittal doming)" ,)	

text( specimen.info$Length.height ~ specimen.info$Carapace.plastron_width , 
      labels = specimen.info$unique.species.numbers , 
      col = ifelse(specimen.info$Hand_webbing==4,'white','black'),  
      cex = 0.5 )


legend('topleft',legend=levels(as.factor(specimen.info$Hand_webbing)),pt.bg=hand_cols,
       pch=21,pt.cex=1.5,cex=1,
       bty='n',ncol=1)


#Coronoal doming plot
plot(specimen.info_to_keep$Carapace_width_height ~ specimen.info_to_keep$Carapace.plastron_width, 
     pch = 21,  bg = hand_cols [as.numeric(as.factor(specimen.info_to_keep$Hand_webbing))] , 
     cex = 2.1,
     xlab = "Carapace width-plastron width ratio" , ylab = "Carapace width-height ratio (coronal doming)" )	

text( specimen.info_to_keep$Carapace_width_height ~ specimen.info_to_keep$Carapace.plastron_width , labels = specimen.info_to_keep$unique.species.numbers , 
      col = ifelse(specimen.info_to_keep$Hand_webbing==4,'white','black'),  
      cex = 0.5 )

legend('topleft',legend=levels(as.factor(specimen.info_to_keep$Hand_webbing)),pt.bg=hand_cols,
       pch=21,pt.cex=1.5,cex=1,
       bty='n',ncol=1)
...
species_means <- specimen.info[,c("Species","Carapace.plastron_width","Length.height","Carapace_width_height","ecology","Hand_webbing")]
...
hand_webbing <- c() #unique hand webbing categories
...
for ( i in 1:nrow(species_means)){
  for ( j in 1:length(unique.species)){
    ...
    sp.temp <- unique.species[j]
      ...
      CW_PW_ratio[j] <- mean ( species_means[ grepl(sp.temp,species_means$Species) , "Carapace.plastron_width" ],na.rm=T   )
      coronal_doming_ratio[j] <- mean ( species_means[ grepl(sp.temp,species_means$Species) , "Carapace_width_height" ] , na.rm = T  )
      sagittal_doming_ratio[j] <- mean ( species_means[ grepl(sp.temp,species_means$Species) , "Length.height" ]  , na.rm = T )
      ...
      hand_webbing[j] <- unique ( species_means[ grepl(sp.temp,species_means$Species) , "Hand_webbing" ]   )
            
    }
  }

names(CW_PW_ratio) <- names(coronal_doming_ratio) <- names(sagittal_doming_ratio) <- names(ecologies) <- names(species) <- names(hand_webbing) <-  unique.species

#bind them all together
species_means  <- data.frame(species,CW_PW_ratio, coronal_doming_ratio, sagittal_doming_ratio, ecologies, hand_webbing)

#fossil measurements  
fossil_measurements <- subset(specimen.info,ecology=='fossil')
fossil_measurements <- fossil_measurements[,c('Species',"Carapace.plastron_width","Carapace_width_height","Length.height",'ecology','Hand_webbing')]
...
## PLOT USING HAND WEBBING DEGREE ##

pdf('sagittal_doming_left_coronal_doming_right_means_webbing.pdf', useDingbats = F, 
    width = 18, height = 9)
...
#Sagittal doming plot
plot(species_means_and_fossils$sagittal_doming_ratio ~ species_means_and_fossils$CW_PW_ratio, 
     pch = 21, bg = hand_cols[as.numeric(as.factor(species_means_and_fossils$hand_webbing))], 
     cex = 2.1, xlab = "Carapace width-plastron width ratio" ,
     ylab = "Carapace length-height ratio (sagittal doming)" )	

#Below line for text identifying specimens
text(species_means_and_fossils$sagittal_doming_ratio ~ species_means_and_fossils$CW_PW_ratio,
     col=ifelse(species_means_and_fossils$hand_webbing==4,'white','black'), 
     lab = as.numeric(as.factor(species_means_and_fossils$species)), cex=0.5)
...
#Coronoal doming plot. For this plot, we use the 'data pfda' object, as it contains only the species with 'true' measurements, i.e., with corrected ratios
plot(species_means_and_fossils$coronal_doming_ratio ~ species_means_and_fossils$CW_PW_ratio, 
     pch = 21, bg = hand_cols[as.numeric(as.factor(species_means_and_fossils$hand_webbing))], 
     cex = 2.1, xlab = "Carapace width-plastron width ratio" ,
     ylab = "Carapace width-height ratio (coronal doming)" )	

#Below line for text identifying specimens
text(species_means_and_fossils$coronal_doming_ratio ~ species_means_and_fossils$CW_PW_ratio,
     col=ifelse(species_means_and_fossils$hand_webbing==4,'white','black'), 
     lab = species_means_and_fossils$numbers, cex=0.5)
